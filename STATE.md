...
**Last updated:** May 7, 2026 ~08:15 EDT (post round-1 demo-feedback ships: MI-101-reorg + MI-401-v2 + MI-101-reorg-v2)
**Updated by:** Lead (CC) on `mi-demo-seed` branch (HEAD parity with `demo-banner`). Wed 5/6 → Thu 5/7 session continued: (12-19) earlier ships + sanitization + v3 + reconciliation per prior STATE versions; (20) `7ea8e32` doc-sync v3; (21) `88b9fef` doc-sync reconcile + Lesson 8 banked; (22) `8ddf416` **MI-101-reorg** (kill Out of Order, hide Assessment under Test Pit, +26/-19); (23) `812c3a5` **MI-401-v2** (GIS / Restorations sub-tab toggle + read-only Restorations aggregate, post Q-401v2-a/b ratification, +174/-28); (24) `d02ede9` **MI-101-reorg-v2** (consolidate Submit Phase grid to 4 tiles via sub-pills in Service Work + Restoration, +40/-4). Round 1 of Jorge's first-click-test feedback (`MI_DEMO_FEEDBACK_round1` sections A + B) now closed end-to-end. Lesson 9 banked: when a directive references a UI element by remembered label, grep the literal phrase before treating it as ground truth (prevented a 30-min speculative build).
...
**~83% of v0.1 scope complete by session count** (Thu 5/7 ~08:15 EDT close, +2% from ~06:45 EDT: round-1 demo-feedback follow-ups all shipped — MI-101-reorg simplified Submit Phase grid 8→6 tiles, MI-401-v2 added Restorations aggregate sub-tab, MI-101-reorg-v2 further consolidated 6→4 tiles via sub-pills. Demo presentation cleaner; click-test asks closed except section C tapcard polish which awaits Jorge specs).
...
| **MI-101-reorg + MI-101-reorg-v2 Submit Phase tile consolidation** | **CLOSED Thu 5/7 ~07:30 + ~08:00 EDT** | Two commits on `demo-banner` (FF to `mi-demo-seed`): `8ddf416` (+26/-19, kill Out of Order + hide Assessment + Test Pit inline link) and `d02ede9` (+40/-4, hide Tapcard + GIS/Docs + parent-form sub-pills). Per MI_DEMO_FEEDBACK round 1 section B + Jorge's 4-tile follow-up directive. Visible Submit Phase grid: Test Pit / Service Work / Restoration / No Work. Hidden tiles (Assessment, Tapcard, GIS/Docs) preserve all existing phase enum routing — write paths unchanged, History view unaffected. Sub-pills inside parent forms invoke hidden-tile click() via three helpers (`openAssessmentFromTestPit`, `openTapcardFromServiceWork`, `openGisDocsFromRestoration`). Out of Order tile + handler + `phase_submissions.out_of_sequence`-write-path entirely removed (column kept in schema; submit payload hardcodes `false`). **Q-101r-c resolved-by-discovery**: Partial LSL already lived as `<option value="PLSL-R">` inside Service Work form's `f-wo-code` dropdown; no separate tile to fold. Lesson 9 banked: grep literal phrase before treating remembered-label directives as ground truth. |
...
| MI-DEMO-UI v3 (firm_safe_to_display gate on user-role chrome + signup toast) | **CLOSED Thu 5/7 ~06:30 EDT** | Commit `ec1f981` on `demo-banner` (FF-merged to `mi-demo-seed`). +19/-3 lines on `index.html` across 5 surgical Lead Edit-tool calls. New `currentFirmSafeToDisplay` global captured in `initApp` from `profile.firms.firm_safe_to_display`, reset in `logout`. Sidebar `.user-role` line: firm name renders only when `currentFirmSafeToDisplay === true`; otherwise '—'; super_admin badge always renders. Signup confirmation toast: dropped firm name (flag not fetchable at signup time post-MI-203 step 3 anon-firms-read drop); replaced with generic "Account created! Check your email to confirm and sign in." Two variables for one column kept intentionally separate: `currentFirmIsDemo` drives banner + pitch-mode scope, `currentFirmSafeToDisplay` drives firm-name display gate — when CP eventually flips the flag for case-study purposes, the banner-vs-name-display split is a one-line change. **Architectural pattern locked in code comments:** canonical "redact firm identity in UI" gate; new customer onboards → flip `firms.firm_safe_to_display = true` on their row → name surfaces. CP currently false → user-role shows '—' on CP sessions. Defense-in-depth posture for prospect-facing demos and shared-screen scenarios. |
...
3. **Thu 5/7 ~03:30–08:15 EDT (eleven commits — `2385c9e` + `558383b` + `74f224a` + `24b430f` + `d64407f` + `d2a0d78` + `a585f67` + `ec1f981` + `7ea8e32` + `88b9fef` + `8ddf416` + `812c3a5` + `d02ede9` + this doc-sync)** — Multiple distinct ships across two phases:

**Phase 1 (~03:30–06:45 EDT, demo-imperative core):** (a) `2385c9e` doc-sync absorbing the Phase 2c-form 8/8 + MI-DEMO-UI v2 + CP firm code rotation arc; (b) `558383b` + `74f224a` Bergen demo property redistribution + handoff-note cleanup; (c) MI-401 Unit 2 (`24b430f`, GIS List tab UI, +396 lines) + MI-404 Unit 2 (`d64407f`, Herald tab UI, +347 lines) — both Lead Edit-tool builds off Buddy Unit 1 backend handoff; (d) `d2a0d78` doc-sync + `a585f67` CLAUDE.md sanitization (Jeff Longberg name dropped + defunct `QUIET-RIVER-58` firm-code literal replaced); (e) **MI-DEMO-UI v3** (`ec1f981`) — `firm_safe_to_display` gate on `.user-role` sidebar chrome + generic signup toast; canonical "redact firm identity in UI" gate; CP user-role shows '—' until flag flipped. Doc-sync v3 + Buddy reconciliation (`7ea8e32` + `88b9fef`); **Lesson 8 banked**.

**Phase 2 (~07:30–08:15 EDT, round-1 demo-feedback follow-ups):** Jorge's first-click-test surfaced specific structural restructure asks. (f) **MI-101-reorg** (`8ddf416`, +26/-19) — Out of Order tile killed; Assessment tile hidden + accessible via inline link inside Test Pit form. (g) **MI-401-v2** (`812c3a5`, +174/-28) — GIS Lists relabeled "GIS / Restorations" with sub-tab toggle; new Restorations sub-tab is read-only aggregate of `phase='restoration'` submissions filtered by `photo_restoration_whiteboard` presence; ratified Q-401v2-a/b before build (both Buddy leans). (h) **MI-101-reorg-v2** (`d02ede9`, +40/-4) — consolidated to 4 visible tiles via sub-pills (Service Work → Tapcard, Restoration → GIS/Docs); Q-101r-c resolved-by-discovery (PLSL-R already an `<option>` inside Service Work form); **Lesson 9 banked** (grep literal phrase before treating remembered-label directives as ground truth).

Buddy worked MI-AUDIT-4 + MI-DEMO-DEPLOY spec in parallel throughout. Lesson 7 (firm_id on every client INSERT) applied in MI-401 + MI-404. Workflow shift mid-Phase-1: Lead switched from "propose Ctrl+F find/replace blocks" to "apply via Edit tool" per Jorge directive after first 11 proposals had accumulated without disk landing — caught at the MI-401 checkpoint commit attempt when `git status` showed no index.html modifications. **Pitch-mode guard tally:** 15 total (10 from MI-DEMO-UI v2 + Phase 2c-form Unit 3, +4 from MI-401, +1 from MI-404; MI-101-reorg + MI-401-v2 + MI-101-reorg-v2 added zero — read-only or routing-preserving structural changes). **All Vercel deployments READY.**
...
- MI-DEMO-UI v3 firm_safe_to_display gate — Thu 5/7 ~06:30 EDT (defense-in-depth privacy on prospect-facing demos)
- MI-101-reorg + MI-101-reorg-v2 Submit Phase grid consolidated to 4 tiles — Thu 5/7 ~07:30 + ~08:00 EDT
- MI-401-v2 GIS / Restorations sub-tab toggle + read-only Restorations aggregate — Thu 5/7 ~07:45 EDT
...
6. **Optional pre-demo polish (if buffer remains, none demo-blocking):** MI-401 Unit 3 (supervisor stats + address autocomplete + CSV export); MI-401 fuzzy-match candidates UI; MI-401 mobile card view; MI-402 Towns/Contractors reference (~30 min); MI-403 Field Guides Tab (~2 sessions); MI-DEMO-UI v3.1 (backend RPC change to surface `firm_safe_to_display` in `lookup_firm_by_code` so signup toast can be selectively gated rather than always-generic).
7. **Identity-display sweep (Buddy v3 carry-forward):** post-v3 ship, run a full sweep of every UI surface that could surface identifying material (firm names, person names, project names) to confirm Lesson 8 coverage. Sidebar `.user-role` + signup toast are gated; verify dashboard chrome / modals / headers / Property Detail / Construction PM surfaces / Luis chat / Herald / GIS Lists / Phase submission lists / report headers don't leak `firm.name` or equivalent. Likely a Buddy parallel-track grep + sample-render audit.
8. **Pattern extension (Buddy v3 carry-forward):** consider whether `person_safe_to_display`, `project_safe_to_display`, etc. flags should join `firm_safe_to_display` as schema-level identity-display gates (e.g., super_admin viewing inspector profiles in a multi-firm reporting context). Punt to a dedicated ticket when first cross-firm reporting surface lands.
9. **Demo pre-flight health check (Buddy v3 carry-forward):** add a `firm_unsafe_displayed` metric to `.coordination/demo_pre_flight_health_check.sql` (would require client-side detection or instrumentation; punt mechanism for now, but the metric belongs in the pre-flight checklist).
...
- v0.1 Compliance Foundation: **~83%** (was ~81%, +2% from round-1 demo-feedback ships closing tile-grid restructure asks + Restorations aggregate sub-tab — demo presentation now click-test-clean)
- v1.0 (commercial-ready Water Utility): **~74%** (was ~72%, +2% — Submit Phase mental-model simplification + cross-property restoration visibility are commercial-ready table stakes)
- Full 7-module platform: **~34%** (was ~33%, +1%)
- Full architected vision (incl. integrations, mobile native, all 7 modules, Luis cross-discipline, residential): **~17%** (was ~16%, +1%)
...
## Banked discipline lessons (Mon 5/5 + Wed 5/6 + Thu 5/7 — 9 lessons total)

Nine locked discipline rules surfaced. Lessons 1-5 from the Mon 5/5 session; Lesson 6 added Wed 5/6 after MI-110 Phase 4 shipped in 1 Buddy turn vs the 6-session brief estimate; Lesson 7 added Thu 5/7 after RLS WITH CHECK silently dropped every Luis conversation INSERT for an unknown duration; Lesson 8 added Thu 5/7 evening after Jorge's click-test screenshot caught CP firm name leaking under his profile name in the sidebar despite pitch mode being active — the read-side display gate was missing even though the write-side pitch-mode guards were comprehensive; Lesson 9 added Thu 5/7 ~08:00 EDT after a "Partial LSL tile to fold" directive turned out to be a `<option>` inside a dropdown, not a tile — caught by grepping the literal phrase before building. Logged at end of STATE.md so future Lead/Buddy reads see them at the bottom of the standing-state digest.
